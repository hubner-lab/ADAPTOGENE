...
config_schema <- function() {
    ...
    list(
        ...
        s("Filter.relatedness", "Max relatedness (pi-hat)", "processing", "Filtering",
          "numeric", FALSE,
          min = 0, max = 1, step = 0.05,
          help = "Optional. Colors/counts related pairs (plink IBD pi-hat) above this in the Processing relatedness histogram (blank = skip). Does NOT remove samples by itself — see 'Relatedness action' below. High-selfing species inflate pi-hat — set from the histogram, not the standard outbred 0.2 default."),
