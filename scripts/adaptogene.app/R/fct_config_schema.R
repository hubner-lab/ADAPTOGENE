...
config_schema <- function() {
    ...
    list(
        ...
        s("GEA.block_mode", "Association mode", "gea", "Methods",
          "select", TRUE,
          choices = c("snp", "block"),
          help = "snp = per-SNP regions; block = LD block entities with WZA p-value pooling"),
        s("GFF.go_field", "GO field in GFF",  "gea", "Methods",
          "text",    FALSE,
          help = "GFF attribute containing GO term IDs. NULL disables enrichment analysis.",
          placeholder = "NULL"),
        # LD Blocks (only used when block_mode = block)
        s("GEA.block.blocks_max_kb", "Max block size (kb)", "gea", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 10,
          help = "Maximum LD block size in kb (plink --blocks-max-kb)"),
        s("GEA.block.adjust", "Block adjust threshold", "gea", "LD Blocks",
          "text", FALSE,
          help = "Block-level p-value adjustment (format: method_alpha, e.g. bonf_0.05, fdr_0.1)",
          placeholder = "bonf_0.05"),
        s("GEA.block.wza.maf_filter", "WZA MAF filter", "gea", "LD Blocks",
          "numeric", FALSE,
          min = 0, max = 0.5, step = 0.01,
          help = "Minimum MAF for SNPs included in WZA aggregation"),
        s("GEA.block.wza.min_snps_per_block", "Min SNPs per block", "gea", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 1,
          help = "Minimum SNPs required per block to compute WZA"),
        s("GEA.block.within_block.method", "Within-block SNP method", "gea", "LD Blocks",
          "select", FALSE,
          choices = c("threshold", "top_n", "lead", "within_order", "all"),
          help = "How to select per-SNP entries within significant blocks"),
        s("GEA.block.within_block.multiplier", "Within-order multiplier", "gea", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 1,
          help = "within_order method: keep SNPs with p < lead_p * multiplier"),
        s("GEA.block.within_block.threshold", "Within-block p cutoff", "gea", "LD Blocks",
          "numeric", FALSE,
          min = 0, max = 1, step = 0.01,
          help = "threshold method: absolute p-value cutoff within a block"),
        s("GEA.block.within_block.top_n", "Within-block top N", "gea", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 1,
          help = "top_n method: keep top N SNPs per significant block"),
        ...
        s("GWAS.block_mode",       "Association mode",    "gwas", "Methods",
          "select",       TRUE,
          choices = c("snp", "block"),
          help = "snp = per-SNP regions; block = LD block entities with WZA p-value pooling"),
        ...
        # LD Blocks (only used when block_mode = block)
        s("GWAS.block.blocks_max_kb",             "Max block size (kb)",      "gwas", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 10,
          help = "Maximum LD block size in kb (plink --blocks-max-kb)"),
        s("GWAS.block.adjust",                    "Block adjust threshold",   "gwas", "LD Blocks",
          "text", FALSE,
          help = "Block-level p-value adjustment (format: method_alpha, e.g. bonf_0.05, fdr_0.1)",
          placeholder = "bonf_0.05"),
        s("GWAS.block.wza.maf_filter",            "WZA MAF filter",           "gwas", "LD Blocks",
          "numeric", FALSE,
          min = 0, max = 0.5, step = 0.01,
          help = "Minimum MAF for SNPs included in WZA aggregation"),
        s("GWAS.block.wza.min_snps_per_block",    "Min SNPs per block",       "gwas", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 1,
          help = "Minimum SNPs required per block to compute WZA"),
        s("GWAS.block.within_block.method",       "Within-block SNP method",  "gwas", "LD Blocks",
          "select", FALSE,
          choices = c("threshold", "top_n", "lead", "within_order", "all"),
          help = "How to select per-SNP entries within significant blocks"),
        s("GWAS.block.within_block.multiplier",   "Within-order multiplier",  "gwas", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 1,
          help = "within_order method: keep SNPs with p < lead_p * multiplier"),
        s("GWAS.block.within_block.threshold",    "Within-block p cutoff",    "gwas", "LD Blocks",
          "numeric", FALSE,
          min = 0, max = 1, step = 0.01,
          help = "threshold method: absolute p-value cutoff within a block"),
        s("GWAS.block.within_block.top_n",        "Within-block top N",       "gwas", "LD Blocks",
          "numeric", FALSE,
          min = 1, step = 1,
          help = "top_n method: keep top N SNPs per significant block"),
        ...
        s("Maladaptation.methods.gradient_forest.combine_gap",
          "SNP combine gap (bp)","maladaptation","Gradient Forest",
          "numeric", TRUE,
          min = 0, step = 10000,
        ...
        s("Maladaptation.methods.gradient_forest.run_label",
          "Run label",          "maladaptation","Gradient Forest",
          "text",    FALSE,
