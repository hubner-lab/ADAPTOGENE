...
#'
#' Displays pipeline-computed trait-vs-trait overlap table (clickable) with
#' Miami plotly rendered on row selection.
#'
...
mod_pairwise_overlap_ui <- function(id) {
    ...
    htmltools::tagList(
                ...
                htmltools::div(
                    ...
                    bslib::layout_columns(
                        ...
                        htmltools::div(
                            ...
                            "Click a row to view the Miami plot for that trait pair."
                        )
                    )
                ),
        ...
        # Miami panel — shown only when a row is selected
...
#' @param coords reactive coords list (chr_offsets + chr_lengths) for genome positioning
#' @noRd
mod_pairwise_overlap_server <- function(id, project_data, coords) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        pairwise_tbl <- shiny::reactive({
            pd <- project_data()
            load_cached(paste0("pairwise_table_", pd$name), function() {
                load_pairwise_table(pd$name)
            })
        })

        collapsed_snps <- shiny::reactive({
            pd <- project_data()
            load_cached(paste0("pairwise_collapsed_", pd$name), function() {
                load_pairwise_collapsed(pd$name)
            })
        })
        ...
        output$pairwise_table <- DT::renderDataTable({
            ...
            safe_datatable(
                ...
                options   = list(
                    ...
                    pageLength = 15,
                    scrollX    = TRUE,
                    order      = list(list(8L, "desc"))  # sort by Score desc
                ),
                rownames = FALSE
            )
        })
        ...
        output$pairwise_miami <- plotly::renderPlotly({
            ...
            # Add cumulative positions (add_cum_pos needs "pvalue" column)
            ...
            window_snps  <- .pairwise_window_snps(dt_a, dt_b, 500000L, 1L)
...
#' Returns list(a_ids, b_ids, windows) where windows is data.table(chr, start, end)
#' @noRd
...
.build_pairwise_plotly <- function(dt_a, dt_b, pair, exact_ids, window_snps, coords) {
    ...
    # Purple rectangles for window overlaps
    ...
    # Gold connector lines for exact matches
