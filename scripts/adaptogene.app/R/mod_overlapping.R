...
mod_overlapping_server <- function(id, project_data) {
    shiny::moduleServer(id, function(input, output, session) {
        ...
        shiny::observe({
            pd      <- project_data()
            rp      <- read_region_params(pd$name)
            saved_g <- get_global_param(rp, MOD_OVERLAP, "combine_gap")
            g <- if (!is.null(saved_g)) as.integer(saved_g)
                 else as.integer(config_get(pd$config, "overlap", "combine_gap",
        ...
        shiny::observeEvent(input$combine_gap, {
            ...
            rp <- read_region_params(pd$name)
            rp <- set_global_param(rp, MOD_OVERLAP, "combine_gap", as.integer(v))
            save_region_params(pd$name, rp)
        }, ignoreInit = TRUE)
        ...
        combine_gap <- shiny::reactive({
            ...
            if (is.null(v) || is.na(v) || v < 0L) {
                pd <- project_data()
                as.integer(config_get(pd$config, "overlap", "combine_gap",
                           default = config_get(pd$config, "association", "combine_gap",
                                                default = 100000L)))
            } else {
                as.integer(v)
