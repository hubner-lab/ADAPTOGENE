...
################### Functions ###################
FUN_pi <- function(vcf_file, samples, inter_dir, vcftools_path, winsize, cpu) {
  ...
  FILENAMES = paste0(inter_dir, 'pop', samples$site %>% unique, '.windowed.pi')
  
  pi_diversity <- lapply(FILENAMES, function(f) {
...
FUN_pi(VCF, samples, INTER_DIR, VCFTOOLS, WINSIZE, CPU) %>%
  fwrite(OUTPUT, sep = '\t')
